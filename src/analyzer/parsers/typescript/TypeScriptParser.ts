    ...
    async parse(rootPath: string, options?: ParserOptions): Promise<GraphData> {
        ...
        for (const sourceFile of sourceFiles) {
            ...
            const processChildNode = (name: string, kind: 'function' | 'class' | 'variable', node: Node, parentId: string) => {
                ...
                node.getDescendantsOfKind(SyntaxKind.CallExpression).forEach(call => {
                    ...
                    if (name.startsWith('use')) {
                        ...
                        if (name === 'useState' || name.endsWith('.useState')) {
                            ...
                            // CallExpression -> VariableDeclaration -> binding? No.
                            // CallExpression -> Initializer -> VariableDeclaration
                ...
                // 2. Props Detection
                let functionNode: FunctionDeclaration | MethodDeclaration | any = node;
                if (kind === 'variable' && Node.isVariableDeclaration(node)) {
                ...
                if (functionNode && functionNode.getParameters) {
                    ...
                    if (params.length > 0) {
                        // Convention: First arg is props
                        const firstParam = params[0];
                        // If destructuring: ({ name, age }: Props)
                        ...
                        if (Node.isObjectBindingPattern(nameNode)) {
                            ...
                            // Standard prop object: (props: Props)
                            // Try to get type
